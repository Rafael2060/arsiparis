...
<div class="modal fade" id="basicModal" tabindex="-1">
    <form id="formHapus" action="<?php echo base_url('SuratKeluarCetak/delete') ?>" method="post" enctype="multipart/form-data">
...
<div class="modal fade" id="modalCari" tabindex="-1">
    <form id="formCari" action="<?php echo base_url('SuratKeluarCetak') ?>" method="get">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="form-group row mx-1 my-1">
                    <label for="no_agenda" class="col-sm-3 col-form-label">Tanggal</label>
                    <div class="col-sm-9">
                        <input class="form-control" class="ml-1" autocomplete="off" id="tanggal" name="tanggal" width="276" value="<?= set_value('tanggal') ?>" />
...
<div class="row d-flex justify-content-between">
    ...
    <div class="col-sm-12 col-md-4 d-flex flex-row-reverse">
        <?php if (cek_staff()) : ?>
            <a style="height:70%" href="<?= base_url('SuratKeluarCetak/create'); ?>" target="_self" class="btn btn-primary" rel="noopener noreferrer">TAMBAH</a>
        <?php endif; ?>
        <a style="height:70%" href="<?= base_url('SuratKeluarCetak/export/?') . 'no_surat=' . $parameter['no_surat'] . '&tanggal=' . $parameter['tanggal'] . '&id_jenissurat=' . $parameter['id_jenissurat']; ?>" target="_self" class="btn btn-success" rel="noopener noreferrer">CETAK</a>
    </div>
</div>
...
<div class="table-responsive">
    <table class="table table-striped">
        ...
        <tbody>
            <?php foreach ($suratkeluarcetaks as $key => $data) : ?>
                <tr>
                    ...
                    <td class="text-left">
                        <div class="btn-group btn-group-sm text-left " role="group" aria-label="Basic example">
                            <a href="<?php echo base_url('SuratKeluarCetak/show/?id=') . $data['id_suratcetak']; ?>" type="button" class="btn bg-gradient btn-info"> <i class="bi bi-eye text-white"></i> </a>

                            <?php if (cek_staff()) : ?>
                                <a href="<?php echo base_url('SuratKeluarCetak/edit/') . $data['id_suratcetak']; ?>" type="button" class="btn bg-gradient btn-warning"><i class="bi bi-pen text-white"></i></a>
                            <?php endif; ?>

                            <?php if (cek_staff()) : ?>
                                <a onclick="javascript:hapussuratperintah('<?php echo $data['id_suratcetak'] ?>','<?php echo $data['no_surat'] ?>')" type="button" data-bs-toggle="modal" data-bs-target="#basicModal" class="btn bg-gradient bg-danger"><i class="bi bi-trash text-white"></i></a>
                            <?php endif; ?>
