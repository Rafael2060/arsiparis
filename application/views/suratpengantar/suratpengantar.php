...
<div class="modal fade" id="basicModal" tabindex="-1">
    <form id="formHapus" action="<?php echo base_url('SuratPengantar/delete') ?>" method="post" enctype="multipart/form-data">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Hapus Data Surat Pengantar</h5>
...
<div class="modal fade" id="modalCari" tabindex="-1">
    <form id="formCari" action="<?php echo base_url('SuratPengantar') ?>" method="get">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Cari Data Surat Pengantar</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
...
<div class="row d-flex justify-content-between">
    ...
    <div class="col-sm-12 col-md-4 d-flex flex-row-reverse">
        <?php if (cek_staff()) : ?>
            <a style="height:70%" href="<?= base_url('SuratPengantar/create'); ?>" target="_self" class="btn btn-primary" rel="noopener noreferrer">TAMBAH</a>
        <?php endif; ?>
        <!-- <a style="height:70%" href="<?= base_url('SuratPengantar/export/?') . 'no_surat=' . $parameter['no_surat'] . '&tanggal_awal=' . $parameter['tanggal_awal'] . '&tanggal_akhir=' . $parameter['tanggal_akhir'] . '&id_jenissurat=' . $parameter['id_jenissurat']; ?>" target="_self" class="btn btn-success" rel="noopener noreferrer">CETAK</a> -->
    </div>
</div>
...
<div class="table-responsive">
    <table class="table table-striped">
        ...
        <tbody>
            <?php foreach ($suratpengantars as $key => $data) : ?>
                <tr>
                    ...
                    <td class="text-left">
                        <div class="btn-group btn-group-sm text-left " role="group" aria-label="Basic example">
                            <a target="_blank" href="<?php echo base_url('SuratPengantar/show/?id=') . $data['id_suratpengantar']; ?>" type="button" class="btn bg-gradient btn-info" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Print"> <i class="bi bi-printer text-white"></i> </a>

                            <?php if (cek_staff()) : ?>
                                <a href="<?php echo base_url('SuratPengantar/edit/') . $data['id_suratpengantar']; ?>" type="button" class="btn bg-gradient btn-warning" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Edit"><i class="bi bi-pen text-white"></i></a>
                            <?php endif; ?>

                            <?php if (cek_kasattahti() || cek_kapolres()) : ?>
                                <?php if ($data['qrcode'] == '') : ?>
                                    <a href="<?php echo base_url('SuratPengantar/tte/') . $data['id_suratpengantar']; ?>" type="button" class="btn bg-gradient btn-success" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="TTE"><i class="bi bi-patch-check text-white"></i></a>
                                <?php endif; ?>
                            <?php endif; ?>

                            <?php if (cek_staff()) : ?>
                                <a onclick="javascript:hapussuratpengantar('<?php echo $data['id_suratpengantar'] ?>','<?php echo $data['no_surat'] ?>')" type="button" data-bs-toggle="modal" data-bs-target="#basicModal" class="btn bg-gradient bg-danger"><i class="bi bi-trash text-white"></i></a>
                            <?php endif; ?>
...
<script>
    function hapussuratpengantar(id, name) {
        // alert(id);
        // alert(name);
