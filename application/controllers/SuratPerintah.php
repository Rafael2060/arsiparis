...
class SuratPerintah extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $config['base_url']             = base_url('SuratPerintah/?') . 'no_surat=' . $no_surat . '&tanggal_awal' . $tanggal_awal . '&tanggal_akhir' . $tanggal_akhir . '&id_jenissurat=' . $id_jenissurat;
        $config['total_rows']           = $this->SuratPerintah_model->total($no_surat, $tanggal_awal, $tanggal_akhir, $id_jenissurat);
        ...
        $data['suratperintahs']     = $this->SuratPerintah_model->suratperintah($no_surat, $tanggal_awal, $tanggal_akhir, $id_jenissurat, $config['per_page'], $offset);
        ...
        $data['title']              = 'Surat Perintah';
        $data['total']              = $config['total_rows'];
        $data['role_id']            = $this->session->userdata('role_id');

        $this->load->view('admin/header', $data);
        $this->load->view('suratperintah/suratperintah');
        $this->load->view('admin/footer');
    }

    public function create()
    {
        $data['title']              = 'Tambah Data Surat Perintah';
        $data['jenissurats']        =  $this->JenisSurat_model->jenissurat('keluar');
    ...
    public function edit()
    {
        ...
        $data['suratperintah']  = $this->SuratPerintah_model->show($id);
        $data['jenissurats']    =  $this->JenisSurat_model->jenissurat('keluar');

        $data['title']          = 'Edit Data Surat Perintah';
    ...
    {
        $id_suratperintah   = $this->input->post('id_suratperintah');
        ...
        $this->SuratPerintah_model->update($id_suratperintah, $data);

        $pesan      = "TTE Surat sudah ditambahkan.";
        pesan($pesan, 'message', 'success');
        redirect(base_url('SuratPerintah'));
    }
    ...
    public function update()
    {
        ...
        $this->form_validation->set_rules('no_surat', 'Nomor Surat', 'required', array('required' => 'Kolom Nomor Surat Perintah tidak boleh kosong.'));
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $pesan      = "Data Surat Perintah sudah diperbarui.";
    ...
    public function store()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $lastId = $this->SuratPerintah_model->store($data);

            $pesan      = "Data Surat Perintah sudah disimpan.";
            // $this->session->set_flashdata('message', '<div style="width:100%" class="alert alert-success alert-dismissible fade show" role="alert">' . $pesan . '<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>');
    ...
    public function show()
    {
        ...
        $data['verifikasis']    = $this->SuratPerintah_model->showVerifikasi($id);
        $data['tahanans']       = $this->SuratPerintah_model->showTahanan($id);
        $data['roles']          = $this->Role_model->role();
        $id_verifikasi          = $this->input->get('id_verifikasi');


        $data['title']          = 'Tampil Surat Perintah';
    ...
    {
        $id_suratperintah      = $this->input->post('idHapus');
        $this->db->where('id_suratperintah', $id_suratperintah)->delete('suratperintah');

        pesan("Data Surat Perintah sudah dihapus.", 'message', 'success');
    ...
    {
        $data3 = explode(",", $data2);
        $result = $this->SuratPerintah_model->unique($data1, $data3[0], $data3[1], $data3[2]);
        if (count($result) == 0) {
            return TRUE;
        } else {
            return FALSE;
    ...
    public function tahanan()
    {
        ...
        $config['base_url']             = base_url('SuratPerintah/tahanan/' . $id);
        ...
        $data['suratkeluar']            = $this->SuratPerintah_model->show($id);
    ...
    public function export()
    {
        ...
        $suratkeluars = $this->SuratPerintah_model->suratkeluar($no_surat, $no_agenda, $tanggal_dikirim_awal, $tanggal_dikirim_akhir, $id_jenissurat, $id_tahanan, '0', $tolak);
        // dd($suratkeluars);
    ...
    {
        $this->load->view('suratperintah/cetakNotaDinas');
    }
    ...
    public function cetakSuratPerintah()
    {
        ...
        $data['suratperintah']  = $this->SuratPerintah_model->suratperintah();

        $this->load->view('suratperintah/cetakSuratPerintah');
    }
    ...
    public function cetak($id)
    {
        // $id     = $this->input->get('id');
        // dd($id);
        $data['suratperintah']  = $this->SuratPerintah_model->cetaksuratperintah($id);
        // dd($data['suratperintah']);
        $this->load->view('suratperintah/cetakSuratPerintah', $data);
    }
