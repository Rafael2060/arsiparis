...
class SuratPengantar extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $config['base_url']             = base_url('SuratPengantar/?') . 'no_surat=' . $no_surat . '&tanggal_awal' . $tanggal_awal . '&tanggal_akhir' . $tanggal_akhir . '&id_jenissurat=' . $id_jenissurat;
        $config['total_rows']           = $this->SuratPengantar_model->total($no_surat, $tanggal_awal, $tanggal_akhir, $id_jenissurat);
        ...
        $data['suratpengantars']    = $this->SuratPengantar_model->suratpengantar($no_surat, $tanggal_awal, $tanggal_akhir, $id_jenissurat, $config['per_page'], $offset);
    ...
    {
        $data['title']              = 'Tambah Data Surat Pengantar';
        $data['jenissurats']        =  $this->JenisSurat_model->jenissurat('keluar');
    ...
    public function tte()
    {
        ...
        $data['title']          = 'TTE Surat Pengantar';
    ...
    {
        $id_suratpengantar   = $this->input->post('id_suratpengantar');
        ...
        $this->SuratPengantar_model->update($id_suratpengantar, $data);
    ...
    public function update()
    {
        ...
        $this->form_validation->set_rules('no_surat', 'Nomor Surat', 'required', array('required' => 'Kolom Nomor Surat Pengantar tidak boleh kosong.'));
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $pesan      = "Data Surat Pengantar sudah diperbarui.";
    ...
    public function store()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $lastId = $this->SuratPengantar_model->store($data);

            $pesan      = "Data Surat Pengantar sudah disimpan.";
            // $this->session->set_flashdata('message', '<div style="width:100%" class="alert alert-success alert-dismissible fade show" role="alert">' . $pesan . '<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>');
    ...
    public function show()
    {
        ...
        $data['title']          = 'Tampil Surat Pengantar';
    ...
    {
        $id_suratpengantar      = $this->input->post('idHapus');
        $this->db->where('id_suratpengantar', $id_suratpengantar)->delete('suratpengantar');

        pesan("Data Surat Pengantar sudah dihapus.", 'message', 'success');
    ...
    {
        $data3 = explode(",", $data2);
        $result = $this->SuratPengantar_model->unique($data1, $data3[0], $data3[1], $data3[2]);
        if (count($result) == 0) {
            return TRUE;
        } else {
            return FALSE;
    ...
    public function tahanan()
    {
        ...
        $config['base_url']             = base_url('SuratPengantar/tahanan/' . $id);
        ...
        $data['suratkeluar']            = $this->SuratPengantar_model->show($id);
    ...
    public function export()
    {
        ...
        $suratkeluars = $this->SuratPengantar_model->suratkeluar($no_surat, $no_agenda, $tanggal_dikirim_awal, $tanggal_dikirim_akhir, $id_jenissurat, $id_tahanan, '0', $tolak);
        // dd($suratkeluars);
