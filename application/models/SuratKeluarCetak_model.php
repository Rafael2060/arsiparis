 ...
 * summary
 */
class SuratKeluarCetak_Model extends CI_Model
{
    /**
     * summary
     */
    ...
    public function suratkeluarcetak($no_surat = null, $tanggal = null, $id_jenissurat = null, $limit = null, $offset = null)
    {
        ...
        if ($tanggal == '') {
            ...
            $rangeTanggal           = 'tanggal = "' . $tanggal_awal . '"';
        }

        if ($id_jenissurat == '' || $id_jenissurat == null) {
            $queryIdJenis = array();
        } else {
            $queryIdJenis = array('suratkeluar_cetak.id_jenissurat' => $id_jenissurat);
        }

        //dd($tanggal_awal);
        $this->db->select('suratkeluar_cetak.*, 
        jenissurat.nama_jenissurat, jenissurat.tipe');
        $this->db->join('jenissurat', 'on jenissurat.id_jenissurat = suratkeluar_cetak.id_jenissurat');
        $this->db->like('no_surat', $no_surat);
        $this->db->where($queryIdJenis);
        $this->db->where($rangeTanggal);

        $this->db->order_by('suratkeluar_cetak.tanggal', 'desc');
        $this->db->limit($limit);
        $this->db->offset($offset);
        return $this->db->get('suratkeluar_cetak')->result_array();
    }
