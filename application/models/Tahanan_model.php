...
class Tahanan_Model extends CI_Model
{
    ...
    public function tahanan($cari, $limit = null, $offset = null)
    {
        $this->db->select('tahanan.*, kategoritahanan.nama_kategori');
        $this->db->like('nama_tahanan', $cari);
        $this->db->join('kategoritahanan', 'on kategoritahanan.id_kategori=tahanan.id_kategori');
        $this->db->order_by('nama_tahanan', 'asc');
        $this->db->limit($limit);
        $this->db->offset($offset);

        return $this->db->get('tahanan')->result_array();
    }
